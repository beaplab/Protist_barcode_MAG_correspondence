## WARNING. the original script was run under Linux 5.15.0-46-generic, Ubuntu 22.04.1 with x86-64 architecture, on a machine with 32 CPU and 125Gb RAM, with R version 4.1.2 (2021-11-01). This is a computationally demanding procedure which is not supposed to run on the local computers.
...
for (d in c(3:length(colnames(vocabulary)))){
  ...
  for(i in list_of_all[1:(length(list_of_all))]) {
    ...
    fwrite(dataSMAGs_df, paste0(metric, i, sg, "data_SMAGs_upd_ext_gen.tsv"),sep="\t",row.names = T)
    ...
    fwrite(dataV9_df_relabund,paste0(metric, i,  sg, "data_V9_relabund_upd_ext_gen.tsv"),sep="\t",row.names = T)
    ...
    fwrite(data_cor,paste0(metric, i, sg, "all_scores_relabund_upd_ext_gen.tsv"),sep="\t")
    ...
    split(data_cor,data_cor[,"SMAG"]) %>%
      ...
      }) %>% rbindlist() %>% fwrite(paste0(metric, i, sg, "best_3_scores_relabund_upd_ext_gen.tsv"),sep="\t")
